...
async function checkOne(item) {
  ...
  const remoteVer = (remotePkg && remotePkg.version) || ""
  const verNewer = !!(localVer && remoteVer && cmpSemver(remoteVer, localVer) > 0)
  let remoteSha = ""
  if (isSelf || !verNewer) remoteSha = await fetchRemoteSha(full)
...
async function checkUpdates(persist) {
  ...
  if (!items.some(x => x.full_name === SELF_FULL)) {
    items.push({
      full_name: SELF_FULL,
      path: "",
  ...
  const enriched = results.map(row => {
    const hit = catalog.find(x => x.full_name === row.full_name)
  ...
  return {
    ...
    installed: enriched,
    newerCount,
    warnings: enriched.filter(x => x.warning).map(x => x.full_name + ": " + x.warning),
    checkedAt,
...
async function handleHttp(req, res) {
  ...
  if (method === 'GET' && path === '/api/dsh-plugins/catalog') {
    try {
      const items = await loadCatalog()
      const installedSet = new Set(collectInstalledGithub().map(x => x.full_name))
      const plugins = items.slice(0, 400).map(p => {
        const r = apiRow(p)
        r.installed = installedSet.has(r.full_name)
        return r
      })
  ...
  if (method === "GET" && path === "/api/dsh-plugins/installed") {
    try {
      ...
      const installed = collectInstalledGithub().map(row => {
        const hit = catalog.find(x => x.full_name === row.full_name)
