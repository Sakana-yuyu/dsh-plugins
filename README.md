<p align="center">
  <img src="docs/logo.svg" width="96" alt="DSH 插件目录">
</p>

<h1 align="center">DSH 插件目录</h1>

<p align="center">
  按 GitHub stars 排名的 DeepSeek Harness 插件<br>
  由 <a href="https://github.com/Sakana-yuyu">R Sakana</a> 维护 · <a href="https://x.com/Sakana19446R">@Sakana19446R</a> 🐟
</p>

<p align="center">
  <a href="https://sakana-yuyu.github.io/dsh-plugins/">在线目录</a> ·
  <a href="https://github.com/topics/dsh-plugin">topic: dsh-plugin</a>
</p>

## 装进 DSH

```sh
dsh plugin --profile web add "github:Sakana-yuyu/dsh-plugins"
```

在系统终端运行，不要让 Agent 代跑，然后重启。

## 网页能做什么

- 搜索、按分类筛选
- 点卡片看 README
- 点命令即可复制安装指令

## 装好后

打开设置里的「插件库」，或对 Agent 说「帮我找视觉插件」。

## 数据来源

GitHub topic `dsh-plugin` + 官方仓库，工作日自动刷新。分类：官方核心、UI、视觉、TUI、桌面、浏览器、工作流、工具、搜索、开发、精选、其他。

## English

Star-ranked DeepSeek Harness plugin catalog, maintained by [R Sakana](https://github.com/Sakana-yuyu). Live: https://sakana-yuyu.github.io/dsh-plugins/

```sh
dsh plugin --profile web add "github:Sakana-yuyu/dsh-plugins"
```

Run it in a real terminal (not the in-session Agent), then restart DSH.
